...
export default function Dashboard({ user }) {
  ...
  if (!isDemo) {
    return (
      <div>
        <div className="page-hdr"><h1>ダッシュボード</h1><p>実データが蓄積されると自動でグラフが表示されます</p></div>
        <div className="kpi-row kpi-4">
          <div className="kpi-card c-blue"><div className="kpi-label">今月の売上</div><div className="kpi-val">¥0</div><div className="kpi-change" style={{color:'#94A3B8'}}>データなし</div></div>
          <div className="kpi-card c-green"><div className="kpi-label">成約件数</div><div className="kpi-val">0<span>件</span></div></div>
          <div className="kpi-card c-orange"><div className="kpi-label">問い合わせ</div><div className="kpi-val">0<span>件</span></div></div>
          <div className="kpi-card c-purple"><div className="kpi-label">今月の経費</div><div className="kpi-val">¥0</div></div>
        </div>
        <div className="two-col">
          <div className="card">
            <div className="card-head"><h3>月次売上推移</h3></div>
            <div className="card-body"><EmptyState icon="📊" text="売上データを入力すると表示されます" /></div>
          </div>
          <div className="card">
            <div className="card-head"><h3>成約の流入元</h3></div>
            <div className="card-body"><EmptyState icon="🥧" text="成約データを入力すると表示されます" /></div>
          </div>
        </div>
        <div className="card">
          <div className="card-head"><h3>最新案件</h3></div>
          <div className="card-body"><EmptyState icon="📋" text="案件管理から案件を追加してください" /></div>
        </div>
      </div>
    )
  }
